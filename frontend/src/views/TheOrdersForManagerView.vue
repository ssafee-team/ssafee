            ...
            <div class="body-container">
                ...
                <div style="width: 100%; height: 80%; display: flex; justify-content: center; align-items: center;">
                    <div class="order-container" style="width: 420px; height: 96%; overflow-x: hidden; overflow-y: scroll;">
                        ...
                        <div @click="orderClick(index)" :class="{isSelected: arrayTF[index]}" class="order" v-for="(order, index) in ordersMenuSorted" :key="index" style="width: 400px; min-height: 100px; border-left:1px solid black; border-right:1px solid black; border-bottom: 1px dashed black; padding: 10px 0 10px 0;">
						...
						<div v-for=" (combi,idx) in order.options" :key="idx" style="display: flex; font-size: 16px; flex-wrap: wrap;">
							<div style="width: 30px; text-align: right; padding-right: 10px; ">
								{{ combi[1] }}
							</div>
...
onMounted(() => {
	...
	//   console.log('배열의크기', menuSet.size)
	  arrayTF.value = new Array(menuSet.size).fill(false)
    }, 
    (error) => {
      console.log(error)
    });
