        ...
        <div class="form-field">
          ...
          <div class="input-with-error">
            <input
              ...
              @keydown="timeValidation"
              @input="formatTime"
              placeholder="13:00"
            />
            <!-- 현재 시간 이후만 입력 가능 에러 메시지 -->

            <span v-if="deadlineError" class="error-message"
              >시간은 현재 시각 이후만 입력 가능합니다.</span
            >
...
.button-modal {
  ...
  cursor: pointer; /* 마우스 커서를 포인터로 변경 */
  border-radius: 5px; /* 둥근 모서리를 만들기 위해 테두리 반경 설정 */
  font-weight: bold;
}
